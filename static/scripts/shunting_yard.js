...
function fix_str(string)
{
    ...
    {
        if ((ind-1 < 0 || "+=-*/%(".includes(string[ind-1]))  && string[ind] == '-')
        {
...
function get_notation(token)
{
    ...
    for (let ind = 0; ind < token.length; ind++)
    {
        ...
        {
            while (oper_stack.length > 0 && ((fInD(prec, oper_stack[oper_stack.length - 1]) >= fInD(prec, token[ind]) && !fInD(assoc, token[ind])) || (fInD(prec, oper_stack[oper_stack.length - 1]) > fInD(prec, token[ind]) && fInD(assoc, token[ind]))))
            {
